...
plot_profiles <- function(xlist) {
  ...
  xrange <- range(na.omit(data$value)) #edit by EC 2019-08 to fix error
  
  # CASE 1: if there is data for 1st element (ctd), then plot ctd and the rest if exists
  if (l$v1[1]>0) {
   
    plot(xlist$v1$value,-xlist$v1$depth, main=xlist$label, type="b", xlab=xlist$xlab, ylab="Depth[m]", xlim=as.numeric(xrange))
  ...
  if (l$v1[1]==0 & l$v2[1]==0) {
    ...
    plot(xlist$v3$value,-xlist$v3$depth, main=xlist$label, type="b",lty=3, pch=15, col="blue",xlab=xlist$xlab, ylab="Depth[m]", xlim=as.numeric(xrange))
