...
# data directory
wd='E:/BioChem QC/2013+/' 

# qc directory (with IML QC output)
qc_folder='E:/BioChem QC/2013_QC/IML_QC_output/'




# create flag review table

##pull flagged data from BCD files
##create table for manual flag review



#find all flagged BCD

bcd_all_fn <- list.files(wd, pattern = 'BCD_flagged.csv', recursive = TRUE, full.names = TRUE)


#gordana's header format
...
for (i in 1:length(bcd_all_fn)){
  ...
  bcd_flags <- bcd %>%
    dplyr::filter(., DIS_DETAIL_DATA_QC_CODE >= 1) %>%
    dplyr::select(.,MISSION_DESCRIPTOR, EVENT_COLLECTOR_EVENT_ID, 
                  EVENT_COLLECTOR_STN_NAME	,DIS_HEADER_SLAT	,
                  DIS_HEADER_SLON, DIS_HEADER_SDATE	, 
                  DATA_TYPE_METHOD	, DIS_DETAIL_DATA_VALUE,
                  DIS_DETAIL_DATA_QC_CODE, DIS_DETAIL_COLLECTOR_SAMP_ID	,
                  DIS_SAMPLE_KEY_VALUE	) %>%
    dplyr::mutate(., REVIEWED_DATA_QC_CODE = '') %>%
    dplyr::mutate(., COMMENT = '')
  
  
  ###AUTO FLAG:: ELECTRODE INCONSISTENCY####
  electrode <- c('O2_Electrode', 'O2_Electrode_mll')
  
  bcd_flags$REVIEWED_DATA_QC_CODE[bcd_flags$DATA_TYPE_METHOD %in% electrode] <- 4
  bcd_flags$COMMENT[bcd_flags$DATA_TYPE_METHOD %in% electrode] <- 'All electrode data flagged for known inconsistencies'
  
  cat(paste(length(bcd_flags$REVIEWED_DATA_QC_CODE[bcd_flags$DATA_TYPE_METHOD %in% electrode]), 'Electrode flags added to ', bcd_flags$MISSION_DESCRIPTOR[1], '\n'))
  
  
  ###WRITE NEW FLAG REVIEW TABLE####
  
  filename <- substr(bcd_fn, 1, nchar(bcd_fn)- 4)
  
  
  write.csv(bcd_flags, file = paste0(filename, '_review.csv'), quote = FALSE, row.names = FALSE)
}




# plot flagged profiles
# generate flag report

# plot flagged profiles using BC flagged file
#edited by E. Chisholm for groundfish reboot


# load flagged BCD file
missions <- list.files(wd)

for (ii in 1:length(missions)){
  
  mission <- missions[ii]
  
  fn=file.path(wd,mission,paste0(mission,"_BCD_flagged.csv"))
  ...
  for (i in 1:length(fe)) {
    ...
    if (length(flag)>0){
      ...
      sflagged=merge(s1,s2,by="sample_key", all = TRUE) #changed from all.x = TRUE
      ...
      #no ctd files
      print(rbind(qcrep[ltp1,]), row.names=F)
      cat("\n")
      cat("\n")
      cat("\n")
    ...
    if (length(odata)>0){
      ...
      xrange=c(min(as.numeric(dp$value[c(o2ctd,o2e,o2w)])),max(as.numeric(dp$value[c(o2ctd,o2e,o2w)]))+1)
      ...
      if (length(flag)>0){
        ...
        print(rbind(qcrep[ltp1,]), row.names=F,na="")
        cat("\n")
        cat("\n")
        cat("\n")
    ...
    if (length(c(chl,chlctd))>0) {
      ...
      if (length(flag)>0){
        ...
        # make a dataframe of flagged data
        cflagged=merge(c1,c2,by="sample_key", all.x=T, all.y = TRUE)
