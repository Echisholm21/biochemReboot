...
#groundfish cruises
mission_list <- list.files('E:/BioChem QC/groundfish_data/')

rem <- grep(mission_list, pattern = 'READ ME')
if(length(rem) > 0 ){
  mission_list <- mission_list[-rem]
}
...
for(i in 1:length(mission_list)){
  ...
  wdp=file.path('E:/BioChem QC/groundfish_data',mission)
  ...
  RODBC_write_df2oracle(con = con,df = bcs1,oracle_table = "GROUNDFISH_MISSIONS_BCS", format_table = bcs_formats)
  
  # write BCD file to oracle table
  RODBC_write_df2oracle(con,df = bcd1,oracle_table = "GROUNDFISH_MISSIONS_BCD", format_table = bcd_formats)
